...
from easyrobust.attacks.robustkdattack import Attack
...
def pgd_generator(images, target, model, teacher, vqgan_aug, attack_type='Linf', eps=4/255, attack_steps=3, attack_lr=4/255*2/3, random_start_prob=0.0, targeted=False, attack_criterion='regular', use_best=True, eval_mode=True):
    ...
    elif attack_criterion == 'robustkd':
        attack_criterion = Attack()
    ...
    for _ in range(attack_steps):
        ...
        if attack_criterion == 'robustkd' and best_x != None:
            adv_xrec = reconstruct_with_vqgan(best_x, vqgan_aug)
            adv_losses = attack_criterion(model(images), model(adv_xrec), teacher(images), target)
        else:
