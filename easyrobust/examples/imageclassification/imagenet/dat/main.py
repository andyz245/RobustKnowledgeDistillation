...
parser.add_argument('--teacher', default='resnet50', type=str, metavar='MODEL',
...
def main():
    ...
    train_transform = teacher.train_preprocess
    val_transform = teacher.val_preprocess
    
    print(train_transform)
    print(val_transform)

    # create the train and eval datasets
    #dataset_train = CIFAR100(download=True,root="./data",transform=train_transform)
    #dataset_eval = CIFAR100(root="./data",train=False,transform=val_transform)
    
    #dataset_train = ImageNet(split="train", transform=train_transform, check_data=True, miniset=False)
    #dataset_eval = ImageNet(split="val", transform=val_transform, check_data=True, miniset=False)
    
    dataset_train = load_dataset("imagenet-1k", split="train")
    dataset_eval = load_dataset("imagenet-1k", split="validation")
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            '''
            if epoch == 299:
                mCE_cifar100("/ceph-jd/pub/jupyter/haohan1/notebooks/workspace/RobustDistillation/data/cc2", model, 'cuda', dataset_eval)
                '''
            ...
            if saver is not None:
                ...
                #torch.save(model.state_dict(), output_dir + "/epoch"  + str(epoch) + ".pth.tar")
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        adv_input = pgd_generator(xrec, target, model, teacher, vqgan_aug, attack_type='L2', eps=0.1, attack_steps=1, attack_lr=0.1, random_start_prob=0.8, use_best=False, attack_criterion='mixup', eval_mode=False)
        ...
        with amp_autocast():
            ...
            loss += loss2
            #loss_terms.append(loss2)
            teacher_adv = loss_fn(teacher_output2, target)
            teacher_on_adv.append(teacher_adv)
            '''
